...
const PopularItems = ({ orders, foodItems }) => {
  ...
  const itemStats = orders.reduce((acc, order) => {
    order.items.forEach(item => {
      ...
      acc[item.foodId].totalOrdered += item.quantity;
      acc[item.foodId].totalRevenue += item.price * item.quantity;
              ...
              return (
                <div key={item.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-amber-700">{item.totalOrdered}</p>
                ...
                <div key={item.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                  <div className="flex items-center space-x-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{item.name}</p>
                        <p className="text-sm text-gray-500">{item.totalOrdered} units sold</p>
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
